...
function copyRecurringFromPrev(state, monthId) {
  ...
  // Copy recurring receipts not already present
  ...
  prev.expenses.filter(e => e.recurring).forEach(e => {
    if (!current.expenses.find(ce => ce.description === e.description)) {
      current.expenses.push({ ...e, id: genId('exp'), status: 'pending', date: monthId + '-10' });
    }
  });
}
...
function buildReceiptForm(r = {}) {
  ...
  return `
    ...
    <div class="form-group" style="flex-direction:row;align-items:center;gap:var(--space-3)">
      <input type="checkbox" id="rec-recurring" ${r.recurring ? 'checked' : ''} />
      <label class="form-label" for="rec-recurring" style="margin:0">Recorrente (copiar para próximos meses)</label>
    </div>
  `;
}
...
function buildExpenseForm(e = {}) {
  ...
  return `
    ...
    <div class="form-group" style="flex-direction:row;align-items:center;gap:var(--space-3)">
      <input type="checkbox" id="exp-recurring" ${e.recurring ? 'checked' : ''} />
...
function getReceiptValues() {
  return {
    ...
    recurring: document.getElementById('rec-recurring')?.checked ?? false,
  };
}
