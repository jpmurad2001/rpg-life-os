...
export function loadState() {
  try {
    ...
    // Future: migration logic here (schema_version checks)
    return parsed;
  } catch (err) {
...
/**
 * Award XP to a specific attribute. Handles attribute level-ups.
 * Returns { state, attrLeveledUp: bool, attr: string, newValue: number }
 */
export function awardAttributeXP(state, attr, amount) {
  if (!ATTR_KEYS.includes(attr)) return { state, attrLeveledUp: false };

  const a = state.player.attributes[attr];
  a.xp_pool += amount;

  let attrLeveledUp = false;
  while (a.xp_pool >= a.xp_per_level) {
    a.xp_pool   -= a.xp_per_level;
    a.value     += 1;
    a.xp_per_level = Math.floor(a.xp_per_level * 1.2); // increasing cost
    attrLeveledUp = true;
  }

  return { state, attrLeveledUp, attr, newValue: a.value };
}
...
export function completeTask(state, weekId, taskId) {
  ...
  const attrAmount = Math.floor(xpGained / 2);
  ...
  // Boss link
  if (task.boss_id) {
    attackBoss(s2, task.boss_id, task.id);
  }

  // Check achievements
  checkAchievements(s2);

  return { state: s2, xpGained, attrGained: attrAmount, leveledUp, newLevel };
}
